...
require(['echarts', 'chroma'], function(echarts, chroma) {
  ...
  var series = [];
  for (label in data) {
    var values = data[label].map(function(item){return item.value});
    var maxValue = Math.max.apply(Math, values);
    var minValue = Math.min.apply(Math, values);
    var basicColor = colorGenerator(label);
    basicColor = chroma(basicColor).alpha(0.5).css();

    series.push({
      name: label,
      type: 'scatter',
      coordinateSystem: 'geo',
      data: convertData(data[label]),
      symbolSize: function (val) {
        return  MIN_SIZE + (val[2] - minValue) / (maxValue - minValue) * MAX_SIZE;
      },
      label: {
        normal: {
          formatter: '{b}',
    ...
    series.push({
      ...
      type: 'effectScatter',
      coordinateSystem: 'geo',
      data: convertData(data[label].sort(function (a, b) {
  ...
  option = {
    ...
    title: {
      text: _params.title ? _params.title : '',
